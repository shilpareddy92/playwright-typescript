...
- Name: NaveenTests/16_autowait.spec.ts >> auto wait 
...
```ts
  ...
  22 | 
  23 |   await page.locator("input[name='agreeTerms']").check();
> 24 |   await page.locator("xpath=input[(name = 'first_name_')]").click({ timeout: 7000 });
     |                                                             ^ TimeoutError: locator.click: Timeout 7000ms exceeded.
  25 | });
  26 | 
```