...
> 65 |   expect(jsonPOSTAPIResponse.booking.additionalneeds).toBe(values[3]);
     |                                                       ^ Error: expect(received).toBe(expected) // Object.is equality
  66 | 
  67 |   expect(jsonPOSTAPIResponse.booking.bookingdates.checkin).toBe(values[4]);
  68 |   expect(jsonPOSTAPIResponse.booking.bookingdates.checkout).toBe(values[5]);
  69 | });
  70 | 
```