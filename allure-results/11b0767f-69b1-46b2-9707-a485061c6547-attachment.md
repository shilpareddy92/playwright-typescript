...
- Name: NaveenTests/16_autowait.spec.ts >> auto wait 3
- Location: tests/NaveenTests/16_autowait.spec.ts:44:5
...
```ts
  ...
  42 | });
  43 | 
  44 | test("auto wait 3", async () => {
  45 |   const browser: Browser = await chromium.launch({
  46 |     headless: false,
  47 |     channel: "chrome",
  48 |   });
  49 |   const page: Page = await browser.newPage();
  50 |   //page.setDefaultTimeout(15000);
  51 | 
  52 |   await page.goto("https://classic.freecrm.com/register/");
  53 | 
  54 |   await page.locator("input[name='agreeTerms']").check();
> 55 |   await page.locator("xpath=input[(name = 'first_name_')]").click();
     |                                                             ^ TimeoutError: locator.click: Timeout 8000ms exceeded.
  56 | });
  57 | 
```