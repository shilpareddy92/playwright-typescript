...
- Name: NaveenTests/16_autowait.spec.ts >> auto wait 2
- Location: tests/NaveenTests/16_autowait.spec.ts:30:5
...
Call log:
  - waiting for locator('xpath=input[(name = \'first_name_\')]')
...
```ts
  ...
  12 | 
  13 | test.use({
  14 |     actionTimeout:8000
  15 | })
  16 | test("auto wait ", async () => {
  17 |   const browser: Browser = await chromium.launch({
  18 |     headless: false,
  19 |     channel: "chrome",
  20 |   });
  21 |   const page: Page = await browser.newPage();
  22 |   //page.setDefaultTimeout(15000);
  23 | 
  24 |   await page.goto("https://classic.freecrm.com/register/");
  25 | 
  26 |   await page.locator("input[name='agreeTerms']").check();
  27 |   
  28 | });
  29 | 
  30 | test("auto wait 2", async () => {
  31 |   const browser: Browser = await chromium.launch({
  32 |     headless: false,
  33 |     channel: "chrome",
  34 |   });
  35 |   const page: Page = await browser.newPage();
  36 |   //page.setDefaultTimeout(15000);
  37 | 
  38 |   await page.goto("https://classic.freecrm.com/register/");
  39 | 
  40 |   await page.locator("input[name='agreeTerms']").check();
> 41 |   await page.locator("xpath=input[(name = 'first_name_')]").click({ timeout: 7000 });
     |                                                             ^ TimeoutError: locator.click: Timeout 7000ms exceeded.
  42 | });
  43 | 
```