...
import patchAPIRequest from "../../test-data/api_requests/PATCH_API_Request.json"; // to create Dynamic Post API request using PW adnTS
...
test(
  "Create DELETE API Request using playwright & typescript",
  {
    tag: [
      "@PlaywrightWithJenkins",
      "@PlaywrightWithAzureDevOpsPipeline",
      "@PlaywrightWithGitHubActions",
    ],
  },
  async ({ request }) => {
    // Reading json file
    ...
    expect(jsonPOSTAPIResponse.booking.bookingdates.checkout).toBe(
      checkOutDate,
    );
    ...
    const patchAPIRespsonse = await request.patch(`/booking/${bookingId}`, {
      headers: {
    ...
    // Validate status code, status text
    expect(patchAPIRespsonse.status()).toBe(200);
    expect(patchAPIRespsonse.statusText()).toBe("OK");

    // Print GET API response
    const putAPIJSONResponse = await patchAPIRespsonse.json();
    console.log(
      "GET API Response : " + JSON.stringify(patchAPIRespsonse, null, 2),
    );
