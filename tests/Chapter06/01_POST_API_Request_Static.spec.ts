...
test("Create POST API Request using static file in playwright & typescript", async ({
  request,
}) => {
  ...
  expect(jsonPOSTAPIResponse.booking.firstname).toBe(
    "playwright typescript by testers talk",
  );
  expect(jsonPOSTAPIResponse.booking.lastname).toBe(
    "playwright javascript by testers talk",
  );
