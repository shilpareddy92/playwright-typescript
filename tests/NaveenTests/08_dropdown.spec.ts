...
test("Select based Drop Down test", async () => {
  ...
  await page.locator(countryDropDown).selectOption({ value: "AF" });
  await page.selectOption(countryDropDown, { value: "AX" });
  await page.locator(countryDropDown).selectOption({ index: 20 });
  await page.locator(countryDropDown).selectOption({ label: "Egypt" });
